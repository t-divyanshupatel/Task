...
Run after [planning.md](./planning.md) inputs are confirmed. Reproduce the bug, trace root cause, apply minimal fix, verify, and write the deliverable.

You **may edit source and test files** in `repoPath` for the fix. Report writes go to `{proofDir}` and/or `{agentDir}/bug-diagnosis-site/` only.
...
| `symptom` | Yes | User description or `discover` (agent infers from repo) |
| `outputFormat` | Yes | `markdown` or `website` |
| `outputPath` | Yes | `{proofDir}/bug-diagnosis-report.md` or `{agentDir}/bug-diagnosis-site/` |
| `reproHint` | No | Optional shortcut command or test name |
...
If the repo documents a seeded bug, quote it. If only a failing test exists, name the test file and assertion.
...
1. **One bug, one fix** — address only the primary root cause identified in Step 4.
...
1. The same command or test that reproduced the bug in Step 3.
...
## Step 7 — Agent vs manual verification
...
Write to `{proofDir}/bug-diagnosis-report.md`.
...
Build at `{agentDir}/bug-diagnosis-site/`.

#### Bootstrap (do not edit template)

```bash
cp -R Task/agents/frontend/. {agentDir}/bug-diagnosis-site/
cd {agentDir}/bug-diagnosis-site
npm install
```

**Never modify files under `Task/agents/frontend/`** — only files inside `bug-diagnosis-site/`.

#### Required site features

1. **Overview page** — metadata (agent, duration, repo, stack, symptom, fix status, verification result), summary
2. **Reproduction timeline** — numbered steps with before-fix output in expandable panels
3. **Root cause panel** — explanation, primary defect table, defective code snippet, call path table with file citations
4. **Fix panel** — rationale, files changed, diff viewer (syntax-highlighted), fixed code snippet
5. **Verification section** — command, exit code, after-fix output, before vs after comparison cards
6. **Agent vs manual verification** — interactive table (agent column filled, manual column shows `pending`)
7. **Risk assessment** — dimension ratings with color badges
8. **Discovery notes** — files examined, secondary issues, ambiguities
9. **Stats cards / charts** — fix status, risk breakdown, files changed count
10. **Source citations** — copy-to-clipboard for every `path:line`
11. **Responsive UI** — clean layout, dark/light friendly

#### Data layer

Generate `{agentDir}/bug-diagnosis-site/data/bug-diagnosis.json` (or typed TS constants) from diagnosis results. Website must reflect **same completeness** as markdown report.
...
8. **Stack-aware** — use the repo's install, run, and test commands; do not introduce new tooling.
9. **Secondary bugs** — document but do not fix unless they block verifying the primary fix.
